...
-- Robust version that handles existing policies
-- Based on your actual database schema with 47 tables
...
DROP POLICY IF EXISTS "Users can view event analytics" ON event_analytics_cache;
DROP POLICY IF EXISTS "Users can view event recommendations" ON event_recommendations;
DROP POLICY IF EXISTS "Users can view event views" ON event_views;
DROP POLICY IF EXISTS "Users can view own event checkins" ON event_checkins;
DROP POLICY IF EXISTS "Event organizers can view checkins" ON event_checkins;
DROP POLICY IF EXISTS "Users can view own RSVPs" ON event_rsvps;
DROP POLICY IF EXISTS "Event organizers can view RSVPs" ON event_rsvps;
DROP POLICY IF EXISTS "Users can view public event reviews" ON event_reviews;
...
DROP POLICY IF EXISTS "Users can view event ownership history" ON event_ownership_history;

-- Ticket Tables
DROP POLICY IF EXISTS "Users can view public tickets" ON tickets;
DROP POLICY IF EXISTS "Event organizers can manage tickets" ON tickets;
DROP POLICY IF EXISTS "Users can view own tickets" ON ticket_wallet;
DROP POLICY IF EXISTS "Event organizers can view event tickets" ON ticket_wallet;
DROP POLICY IF EXISTS "Users can view own transfers" ON ticket_transfers;
DROP POLICY IF EXISTS "Users can manage own transfers" ON ticket_transfers;
DROP POLICY IF EXISTS "Users can view own ticket scans" ON ticket_scans;
DROP POLICY IF EXISTS "Event organizers can view event scans" ON ticket_scans;
DROP POLICY IF EXISTS "Users can view public ticket tiers" ON ticket_tiers;
DROP POLICY IF EXISTS "Event organizers can manage ticket tiers" ON ticket_tiers;
DROP POLICY IF EXISTS "Users can view public tier badges" ON tier_badges;
DROP POLICY IF EXISTS "Users can view own event badges" ON user_event_badges;
...
DROP POLICY IF EXISTS "Users can view public post comments" ON post_comments;
DROP POLICY IF EXISTS "Users can view own post comments" ON post_comments;
DROP POLICY IF EXISTS "Users can view public post reactions" ON post_reactions;
DROP POLICY IF EXISTS "Users can view own post reactions" ON post_reactions;
...
DROP POLICY IF EXISTS "Users can view public reactions" ON reactions;
...
DROP POLICY IF EXISTS "Users can create stripe customer" ON stripe_customers;
...
DROP POLICY IF EXISTS "Users can create payment intents" ON stripe_payment_intents;
...
DROP POLICY IF EXISTS "Users can create payout accounts" ON payout_accounts;
DROP POLICY IF EXISTS "Users can update own payout accounts" ON payout_accounts;

-- Cart and Promo Tables
...
DROP POLICY IF EXISTS "Users can update own cart holds" ON cart_holds;
DROP POLICY IF EXISTS "Users can view public promo codes" ON promo_codes;
DROP POLICY IF EXISTS "Event organizers can manage promo codes" ON promo_codes;
...
DROP POLICY IF EXISTS "Users can view own creator dashboard widgets" ON creator_dashboard_widgets;
...
CREATE POLICY "Users can view event analytics" ON event_analytics_cache
    FOR SELECT USING (
        auth.uid() IN (
            ...
            WHERE e.id = event_analytics_cache.event_id AND om.role IN ('admin', 'owner')
        )
    );
...
CREATE POLICY "Event organizers can view checkins" ON event_checkins
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM org_members om
            JOIN events e ON e.org_id = om.org_id
            WHERE e.id = event_checkins.event_id AND om.role IN ('admin', 'owner')
        )
    );
...
CREATE POLICY "Event organizers can view RSVPs" ON event_rsvps
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM org_members om
            JOIN events e ON e.org_id = om.org_id
            WHERE e.id = event_rsvps.event_id AND om.role IN ('admin', 'owner')
        )
    );
...
CREATE POLICY "Users can view event ownership history" ON event_ownership_history
    FOR SELECT USING (
        auth.uid() IN (
            SELECT user_id FROM org_members om
            JOIN events e ON e.org_id = om.org_id
            WHERE e.id = event_ownership_history.event_id AND om.role IN ('admin', 'owner')
        )
    );
...
CREATE POLICY "Event organizers can manage ticket tiers" ON ticket_tiers
    FOR ALL USING (
        auth.uid() IN (
            SELECT user_id FROM org_members om
            JOIN events e ON e.org_id = om.org_id
            WHERE e.id = ticket_tiers.event_id AND om.role IN ('admin', 'owner')
        )
    );
...
SELECT 
    ...
    'Optimized RLS policies for all 47 tables, secure functions, and access controls implemented' as message,
