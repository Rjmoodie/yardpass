-- Debug and Fix Search Functions for YardPass (FINAL VERSION)
-- This script safely checks for missing functions and creates them if needed
...
-- ✅ DEBUG: Create basic indexes if they don't exist (SAFE VERSION)
DO $$
BEGIN
    -- Create basic indexes for search performance (only if they don't exist)
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_events_search_basic') THEN
        RAISE NOTICE 'Creating basic search index...';
