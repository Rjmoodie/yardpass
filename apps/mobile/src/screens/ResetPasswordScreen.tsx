...
const ResetPasswordScreen: React.FC = () => {
  ...
  const checkUrlParams = () => {
    try {
      // For mobile, we need to check if we're in a web view or handle deep links
      // This is a simplified version - you might need to adjust based on your navigation setup
      
      // Check if we have any route params that might contain tokens
      const route = navigation.getState()?.routes?.find(r => r.name === 'ResetPassword');
      const params = route?.params as any;
      
      console.log('ResetPassword component - checking for tokens:');
      console.log('Route params:', params);
      
      if (params?.access_token && params?.refresh_token && params?.type === 'recovery') {
        console.log('Setting reset mode - tokens found in route params');
        setAccessToken(params.access_token);
        setRefreshToken(params.refresh_token);
        setIsResetMode(true);
        
        // Establish session with Supabase
        establishSession(params.access_token, params.refresh_token);
      } else {
        console.log('Not setting reset mode - no tokens found');
