  ...
  from: (table: string) => ({
    select: (columns: string = '*') => ({
      ...
      in: (column: string, values: any[]) => ({
        execute: async () => {
          console.log(`Supabase select from ${table} where ${column} in [${values.join(', ')}]`);
          return { data: [], error: null };
        },
      }),
      range: (start: number, end: number) => ({
        execute: async () => {
          console.log(`Supabase select from ${table} range ${start}-${end}`);
...
export const formatResponse = (data: any, error: any) => {
  if (error) {
